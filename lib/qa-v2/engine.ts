...
function isMedicalPlanComparisonOrPricingQuestion(query: string): boolean {
  const lower = stripAffirmationLeadIn(query.trim()).toLowerCase();
  return (
    /\b(compare|comparison|versus|vs\.?|side\s+by\s+side|breakdown|pricing|plan pricing|premium|premiums|per month|monthly|show me the plans|show me the breakdown|show me the prices|show me the premiums|show me the numbers|numbers again|just want to see the plans|just wanna see the plans|just plan pricing|what about just plan pricing|show me the employee\s*\+\s*(?:family|spouse|child(?:ren)?)\s+(?:premiums|prices)|show me the family prices|show me the spouse prices)\b/i.test(lower)
      && /\b(medical|plan|plans|standard hsa|enhanced hsa|kaiser|hmo|coverage tier|coverage tiers|employee\s*\+|spouse|family|whole family|wife|husband|partner|kids?|children)\b/i.test(lower)
  ) || (
    /\b(whole family|my family|family|wife|husband|spouse|partner|kids?|children|household|employee\s*\+\s*(?:family|spouse|child(?:ren)?)|cover myself)\b/i.test(lower)
      && /\b(price|prices|pricing|premium|premiums|per month|monthly|show me|breakdown)\b/i.test(lower)
  );
}
...
function isMedicalPremiumReplayQuestion(query: string): boolean {
  const lower = stripAffirmationLeadIn(query.trim()).toLowerCase();
  return /\b(show\s+me\s+the\s+numbers(?:\s+again)?|show\s+me\s+the\s+monthly\s+numbers|show\s+me\s+how\s+much\s+i\s+have\s+to\s+pay\s+each\s+month|monthly\s+premiums?|per\s+month\s+on\s+each\s+plan|how\s+much\s+will\s+my\s+premium\s+be|how\s+much\s+(?:the\s+)?premiums?\s+are|what\s+are\s+the\s+premiums?|what\s+would\s+the\s+premium\s+be|show\s+me\s+.*pricing|pricing\s+for\s+employee|show\s+me\s+the\s+employee\s*\+|show\s+me\s+the\s+employee\s*\+\s*(?:family|spouse|child(?:ren)?)\s+(?:premiums|prices)|employee\s*\+\s*(?:family|spouse|child(?:ren)?)\s+pricing|employee\s*\+\s*(?:family|spouse|child(?:ren)?)\s+premiums|employee\s*\+\s*(?:family|spouse|child(?:ren)?)\s+prices|just\s+plan\s+pricing|what\s+about\s+just\s+plan\s+pricing|whole\s+family\s+pricing|show\s+me\s+the\s+family\s+prices|show\s+me\s+family\s+prices|family\s+prices|show\s+me\s+the\s+spouse\s+prices|spouse\s+prices|prices?\s+for\s+premiums?|show\s+me\s+the\s+prices|show\s+me\s+the\s+premiums|show\s+me\s+the\s+breakdown\s+of\s+(?:(?:each\s+of\s+)?(?:those|these)\s+plans?|each\s+plan)|breakdown\s+of\s+(?:(?:each\s+of\s+)?(?:those|these)\s+plans?|each\s+plan)|just\s+wanna\s+see\s+(?:the\s+)?plans?)\b/i.test(lower);
}
